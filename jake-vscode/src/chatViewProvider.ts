...
function getApiBase(): string {
  return vscode.workspace.getConfiguration('jakeSquad').get<string>('apiBaseUrl', 'http://localhost:8000');
}
...
export class ChatViewProvider implements vscode.WebviewViewProvider {
  public static readonly viewType = 'jakeSquadChat';
  private _view?: vscode.WebviewView;
  private _currentPersona = '제이크';
  ...
  async switchPersona(name: string) {
  ...
  private async _loadHistory(persona: string) {
  ...
  private async _handleSend(text: string) {
    if (!this._view || !text.trim()) return;
