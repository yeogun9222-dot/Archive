...
async function getWithTimeout(path: string): Promise<any> {
  ...
  try {
    ...
    if (!res.ok) return { error: `jake-agent 응답 오류: HTTP ${res.status}` };
    return await res.json();
  } catch (e: any) {
    if (e.name === 'AbortError') return { error: `jake-agent 응답 시간 초과 (${TIMEOUT_MS / 1000}초)` };
    return { error: `jake-agent 연결 실패: ${e.message}` };
  } finally {
    clearTimeout(timer);
  }
...
server.registerTool(
  'ask_alpha_squad',
  {
    ...
    description:
      '안건을 Alpha Squad 전체 회의에 올립니다. 제이크 COO가 관련 팀원들의 의견을 수렴하여 종합 답변을 정리합니다.',
    inputSchema: {
      message: z.string().describe('회의에 올릴 안건/질문'),
    },
  },
  async ({ message }) => {
...
server.registerTool(
  'clear_persona_history',
  {
    title: '팀원 대화 기록 초기화',
    description: '특정 팀원 또는 Alpha Squad 전체방의 저장된 대화 기록을 삭제합니다.',
    inputSchema: {
      persona: z.string().describe('초기화할 팀원 이름, 또는 전체방은 "alpha-squad"'),
    },
  },
  async ({ persona }) => {
    const controller = new AbortController();
    ...
    try {
      ...
      clearTimeout(timer);
      if (!res.ok) return { content: [{ type: 'text', text: `초기화 실패: HTTP ${res.status}` }] };
...
server.registerTool(
  'get_persona_history',
  {
    ...
    inputSchema: {
      persona: z.string().describe('조회할 팀원 이름, 또는 전체방은 "alpha-squad"'),
      limit: z.number().optional().describe('조회할 메시지 수 (기본 20)'),
    },
  },
  async ({ persona, limit }) => {
    ...
    if (messages.length === 0) return { content: [{ type: 'text', text: '대화 기록 없음' }] };
    const text = messages
      .map((m: any) => `${m.role === 'user' ? '대표님' : persona}: ${m.content}`)
      .join('\n');
    return { content: [{ type: 'text', text }] };
  }
);
