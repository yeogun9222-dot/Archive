...
export const MetricCard = ({ title, value, suffix, delay = 0 }: { title: string, value: number, suffix: string, delay?: number }) => (
  ...
  >
    ...
    <div className="relative h-full bg-gradient-to-br from-[#800000] via-[#660000] to-[#4a0000] border border-luxury-gold/30 rounded-2xl p-3 lg:p-10 flex flex-col items-center justify-center overflow-hidden shadow-[0_15px_40px_-10px_rgba(0,0,0,0.8)] lg:shadow-[0_30px_70px_-15px_rgba(0,0,0,0.8)] group-hover:border-luxury-gold group-hover:shadow-[0_40px_80px_-15px_rgba(128,0,0,0.3)] transition-all duration-500 min-h-[140px] lg:min-h-[280px]">
      ...
      <div className="relative z-10 text-center">
        <div className="mb-2 lg:mb-8 inline-block">
          <h4 className="text-[8px] lg:text-[16px] font-black text-white/90 uppercase tracking-[0.2em] lg:tracking-[0.6em] drop-shadow-lg group-hover:text-luxury-gold-light transition-colors duration-300">
            {title}
          </h4>
          <div className="w-full h-0.5 lg:h-1 bg-luxury-gold mt-1 lg:mt-4 rounded-full transform scale-x-50 group-hover:scale-x-100 group-hover:bg-white transition-all duration-700 shadow-[0_0_10px_rgba(234,179,8,0.5)]"></div>
        </div>
        <p className="text-2xl lg:text-8xl font-mono font-black text-white tracking-tighter drop-shadow-[0_4px_10px_rgba(0,0,0,0.6)] lg:drop-shadow-[0_8px_20px_rgba(0,0,0,0.6)]">
...
export const PhoneMockup = () => (
  <div className="relative animate-floating-phone" style={{ perspective: '2000px' }}>
    <div className="relative" style={{
      ...
      <div className="w-full h-full p-6 flex flex-col items-center justify-between relative bg-gradient-to-br from-[#3d0a0a] via-[#1a0505] to-[#0a0a0a] z-50">
        <div className="w-full pt-10 text-center">
          <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-luxury-gold/40 to-transparent border border-luxury-gold/30 flex items-center justify-center mx-auto mb-4 backdrop-blur-sm">
            <Brain size={24} className="text-luxury-gold" />
          </div>
          <p className="text-[8px] font-black text-luxury-gold uppercase tracking-[0.4em] mb-1">Neural Core V6</p>
          <p className="text-lg font-serif font-black text-white">LONGRISE</p>
        </div>
        <div className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 space-y-3 backdrop-blur-md">
          <div className="flex justify-between items-center text-white/40">
            <span className="text-[8px] uppercase font-black">System Status</span>
            <span className="text-[8px] text-green-400 font-black flex items-center gap-1">
              <div className="w-1 h-1 bg-green-400 rounded-full animate-pulse"></div> 
              SECURED
            </span>
          </div>
          <div className="h-1 bg-white/10 rounded-full overflow-hidden">
            <motion.div animate={{ x: [-100, 100] }} transition={{ duration: 4, repeat: Infinity, ease: "linear" }} className="w-1/2 h-full bg-gradient-to-r from-luxury-gold/30 via-luxury-gold to-white/20" />
          </div>
          <div className="pt-2 text-center">
            <p className="text-sm font-mono font-black text-white">$142,500,000</p>
            <p className="text-[7px] text-gray-500 uppercase font-black tracking-widest mt-1">Global Liquidity</p>
          </div>
        </div>
        <div className="w-full pb-4 px-2">
          <button className="w-full py-3.5 bg-luxury-gold text-black text-[10px] font-black rounded-xl shadow-[0_10px_20px_rgba(234,179,8,0.3)] uppercase tracking-widest active:scale-95 transition-transform">
            ENTER TERMINAL
          </button>
        </div>
        <div className="absolute bottom-2 left-1/2 -translate-x-1/2 w-24 h-1 bg-white/50 rounded-full"></div>
      </div>
    </div>
  </div>
);
      ...
      <div className="grid grid-cols-1 gap-8 items-center text-center">
        ...
        >
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <button onClick={onStartClick} className="gold-button !px-12 !py-5 flex items-center justify-center gap-3 group relative overflow-hidden text-xs uppercase tracking-widest">
