          ...
          <div className="glass-panel p-6 rounded-2xl space-y-4">
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center text-sm">
                ...
                <span className="text-luxury-gold font-bold">LV.2 (Verified)</span>
              </div>
              <div className="flex justify-between items-center text-sm">
                <div className="flex items-center gap-3 text-gray-400">
                  <Bell size={16} />
        ...
        <div className="lg:col-span-3 space-y-8">
          <div className="glass-panel p-8 rounded-2xl space-y-10">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
              <div className="space-y-6">
                ...
                <div className="space-y-4">
                  {[
                    ...
                    { label: 'Mobile Number', value: '+82 10-1234-5678', icon: Phone },
                    { label: 'Preferred Language', value: 'Korean (KR)', icon: Globe },
                    { label: 'Main Address', value: 'Gangnam-gu, Seoul, KR', icon: MapPin },
                  ].map((info) => (
              ...
              <div className="space-y-6">
                <h3 className="text-xl font-bold flex items-center gap-3">
                  <FileCheck className="text-luxury-gold" />
                  KYC Verification
                </h3>
                <div className="p-8 rounded-2xl bg-gradient-to-br from-luxury-gold/10 to-transparent border border-luxury-gold/20 flex flex-col items-center text-center space-y-6">
                  <div className="w-16 h-16 rounded-2xl bg-luxury-gold flex items-center justify-center">
                    <ScanFace size={32} className="text-black" />
                  </div>
                  <div>
                    <h4 className="font-bold text-white mb-2">Level 3 Verification Pending</h4>
                    <p className="text-xs text-gray-400 max-w-[200px]">Increase your daily withdrawal limit to $2.5M by completing Level 3.</p>
                  </div>
                  <button className="w-full py-4 bg-luxury-gold text-black rounded-xl font-black text-[10px] tracking-widest uppercase hover:scale-105 transition-all">
                    COMPLETE NOW
                  </button>
                  <p className="text-[10px] text-gray-500 italic">* Verification usually takes 2-6 business hours.</p>
                </div>
              </div>
            </div>
            ...
            <div className="pt-10 border-t border-white/5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {[
                  { title: 'Enhanced 2FA', desc: 'Secure with Google Authenticator', active: true },
                  { title: 'Login Protect', desc: 'Alert upon unrecognized login', active: true },
                  { title: 'Anti-Phishing', desc: 'Show personal code in emails', active: false },
                  { title: 'Biometric Access', desc: 'Use FaceID/TouchID on mobile', active: true },
                ].map((pref) => (
                  <div key={pref.title} className="flex items-center justify-between p-6 bg-white/5 border border-white/5 rounded-2xl group transition-all hover:bg-white/[0.08]">
                    <div>
                      <h4 className="text-sm font-bold text-white">{pref.title}</h4>
                      <p className="text-xs text-gray-500">{pref.desc}</p>
                    </div>
                    <div className={`w-12 h-6 rounded-full p-1 transition-colors duration-500 ${pref.active ? 'bg-luxury-gold' : 'bg-gray-700'}`}>
                      <div className={`w-4 h-4 bg-white rounded-full transition-transform duration-500 ${pref.active ? 'translate-x-6 shadow-lg' : 'translate-x-0'}`} />
                    </div>
                  </div>
                ))}
                  ...
                  <button 
                    ...
                    className={`px-6 py-2.5 rounded-xl text-[10px] font-black tracking-widest uppercase transition-all ${
                      displayUser.hasSetTradingPassword 
                        ...
                        : 'bg-luxury-gold text-black hover:scale-105'
                    }`}
                  >
