...
export const NoticesPage = ({ portalData }: { portalData?: any }) => {
  ...
  const liveNewsItems = (portalData?.news && portalData.news.length > 0)
    ? portalData.news.map((item: any, index: number) => ({
        id: index + 1,
        category: item.category === 'PROMOTIONS' ? 'PROMOTION' : item.category === 'MAINTENANCE' ? 'SYSTEM' : item.category,
        isNew: Boolean(item.isNew),
        date: item.date ? item.date.slice(0, 10) : '',
        title: item.title,
        desc: item.desc,
        icon: item.category === 'PROMOTIONS' ? Gift : item.category === 'MAINTENANCE' ? Info : Cpu,
        iconBg: item.category === 'PROMOTIONS' ? 'bg-red-900/30 text-red-500' : item.category === 'MAINTENANCE' ? 'bg-white/5 text-gray-400' : 'bg-blue-900/30 text-blue-500'
      }))
    : newsItems;
    ...
    <div className="pt-32 pb-24 px-6 lg:px-10 max-w-7xl mx-auto space-y-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div className="lg:col-span-3 space-y-6">
          {liveNewsItems.filter((item: any) => activeCategory === 'ALL' || item.category === activeCategory).map((item: any, i: number) => (
            <motion.div 
               key={item.id}
               initial={{ opacity: 0, x: 20 }}
               animate={{ opacity: 1, x: 0 }}
